...
const TailedBeastScreen = () => {
  const [data, setData] = useState<TailedBeast[]>([]);
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      const response = await getData("tailed-beasts", {
        page: pageNumber,
        limit: 20,
      });

      if (response["tailed-beasts"].length === 0) {
        setHasMore(false);
      } else {
        setData((prev) => [...prev, ...response["tailed-beasts"]]);
        setPage(pageNumber);
      }
    } finally {
      setLoading(false);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No Tailed Beast found</Text>
          </View>
        }
        renderItem={({ item }) => {
...
export default TailedBeastScreen;
