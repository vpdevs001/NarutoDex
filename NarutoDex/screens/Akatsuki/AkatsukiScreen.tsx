...
import type { AkatsukiMember } from "../../types/akatsuki-types";
...
type NavigationProp = NativeStackNavigationProp<RootStackParamList, "Akatsuki">;

const AkatsukiScreen = () => {
  const [data, setData] = useState<AkatsukiMember[]>([]);
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      const response = await getData("akatsuki", {
        page: pageNumber,
        limit: 20,
      });

      if (response.akatsuki.length === 0) {
        setHasMore(false);
      } else {
        setData((prev) => [...prev, ...response.akatsuki]);
        setPage(pageNumber);
      }
    } finally {
      setLoading(false);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No Akatsuki member found</Text>
          </View>
        }
        renderItem={({ item }) => {
