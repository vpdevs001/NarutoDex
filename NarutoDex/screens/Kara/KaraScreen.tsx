...
import type { KaraMember } from "../../types/kara-types";
...
const KaraScreen = () => {
  const [data, setData] = useState<KaraMember[]>([]);
  ...
  const fetchData = async (pageNumber: number) => {
    ...
    try {
      ...
      if (response.kara.length === 0) {
        setHasMore(false);
      } else {
        setData((prev) => [...prev, ...response.kara]);
        setPage(pageNumber);
      }
    } finally {
      setLoading(false);
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No Kara member found</Text>
          </View>
        }
        renderItem={({ item }) => {
